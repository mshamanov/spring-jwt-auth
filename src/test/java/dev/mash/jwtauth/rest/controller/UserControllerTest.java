...
import dev.mash.jwtauth.config.SecurityConfiguration;
import dev.mash.jwtauth.entity.User;
import dev.mash.jwtauth.exception.JwtAuthenticationEntryPoint;
...
class UserControllerTest {
    ...
    void givenUserUsesInvalidToken_whenGetWelcome_thenAccessIsForbidden(SignInRequest signInRequest, User user) throws Exception {
        ...
        String token = AuthControllerTest.fetchToken(this.mockMvc, signInRequest);
        token = token.replaceAll(".$", ""); // remove last symbol to change token thus make it invalid
